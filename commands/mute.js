...
client.on('message', async (message) => {
  try {
    ...
    let muteRole = await message.guild.roles
      .fetch('700888264575418378')
      .catch((error) => console.log(`muteRole error: ${error}`))
        ...
        if (!person) {
          ...
          if (!time) {
            ...
            return message.author.send(
              "You didn't specify a time for the mute command! [!mute @user 10s or !mute @user 1d] where 10s = 10 seconds and 1d = 1 day"
            )
          }
          ...
          message.channel.send(
            '<@' + person.user.id + '>' + ` has been muted for ${ms(ms(time))}`
          )

          setTimeout(function () {
            person.roles.remove(muteRole)
            message.channel.send(
              '<@' + person.user.id + '>' + ' has been unmuted.'
            )
          }, ms(time))
        }
      } else {
